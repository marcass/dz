...
public class TextConfiguration implements Configuration {
    ...
    public void setDefaultConfiguration(Configuration conf) {
        ...
        if (oldDefault == null) {
            ...
            // No actions necessary, there were no defaults
            return;
        }

        // Traverse the defaults of the new default until the end is found
        // and append the old default chain to the new one
    ...
    public String getString(String key, String defaultValue) {
        ...
        if (found == null) {
            ...
            // VT: NOTE: This is rather harsh, but enforces the
    ...
    public List getList(String key, List defaultValue) {
        ...
        if (found instanceof String) {
            ...
            for (StringTokenizer st = new StringTokenizer((String) found, ",; "); st.hasMoreTokens();) {
                ...
                // VT: FIXME: Analyze the previous token for the escape
                // symbols so we can include the separators into values as
                // well
    ...
    public void put(String key, Object value) {
        ...
        // Preprocess the value. It may turn out that the value is a comma
        // separated string, which will embarrass us a little.
        ...
        if (value instanceof String && ((String) value).indexOf(",") != -1) {
            ...
            // System.err.println(key + " split: '" + value + "'");

            String valueAsString = (String) value;
            List<String> valueAsList = new LinkedList<String>();

            // VT: FIXME: This way, it's impossible to have a comma in the
            // value. Have to process the escape with a backslash.

            for (StringTokenizer st = new StringTokenizer(valueAsString, ","); st.hasMoreTokens();) {

                String element = st.nextToken().trim();

                // System.err.println(key + " element: '" + element + "'");
        ...
        if (found == null) {
            ...
            // OK, we don't have to fiddle with the sequence
        ...
        if (found instanceof String) {
            ...
            // System.err.println(key + " restructure: '" + found + "'");
            Object first = found;
        ...
        if (found instanceof List) {
            ...
            // OK, there's existing sequence already.
            ...
            // Oh well, it's already there, I'm not going to add a
            // duplicate
        }

        // Now, we know for sure that some object is already set to be the
        // configuration value, and it's not a collection (well, List), so
        // we have to create a collection and add new value to it.
    ...
    protected void duplicateKeyword(String key) {
        ...
        throw new IllegalArgumentException(getClass().getName() + "#" + Integer.toHexString(hashCode()) + ": "
                + "The invocation context for the configuration item "
    ...
    public synchronized URL[] getUrlChain() {
        ...
        // So now we know how big of a chain do we need
